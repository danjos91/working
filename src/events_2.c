...
void		events_vel(t_player *pl, t_sub_ev *se, t_others *ot)
{
	...
	if (se->wsad[0])
	{
	    if (se->wsad[6])
        {
            ot->move_vec[0] += pl->anglecos * (0.5f);// + 0.2 * se->wsad[6]);
            ot->move_vec[1] += pl->anglesin * (0.5f);
        }
		ot->move_vec[0] += pl->anglecos * (0.2f);// + 0.2 * se->wsad[6]);
		ot->move_vec[1] += pl->anglesin * (0.2f);// + 0.2 * se->wsad[6]);
	}
	if (se->wsad[1])
	{
