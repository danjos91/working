...
static int	motion_chk_2(t_sect_ops *op, t_player *pl, int i)//HIT WALL
{
	/* Check where the hole is. */
	op->hole_low = op->sect->neighbors[i] < 0 ? 9e9 : max(op->sect->floor, pl->sectors[op->sect->neighbors[i]].floor);
	// op->hole_low = op->sect->neighbors[i] < 0 ? 9e9 : max(op->sect->floor, pl->sectors[op->sect->neighbors[i]].floor);
