...
void	ceil_floor_light(t_draw_screen_calc *ds, t_player *p, t_game *g)// aqui se realizan algunos calculos importantes en ds
{
...
void	render_ceil_floor(t_draw_screen_calc *ds, t_player *p, t_game *g)//is in render_sector, se puede no usar si es para el techo y para el piso
{
...
void		render( t_tex_i tex_i, t_player *p, t_draw_screen_calc *ds, t_game *g)
{
    ds->i->tn = tex_i.wall;
...
int			color_transoform(int color, float percent)
{
    ...
    color = ((rgb[2] << 24) |(rgb[2] << 16) | (rgb[1] << 8) | rgb[0] << 0);
    return (color);
}
